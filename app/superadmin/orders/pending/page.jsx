...
export default function PendingOrdersPage() {
  ...
  const fetchPendingOrders = async () => {
    try {
      ...
      // Calculate overall stats (only for pending orders)
      const total = pendingOrders.length;
      const pending = pendingOrders.length;
      const processing = 0; // No processing orders in pending view
      const delivered = 0; // No delivered orders in pending view
      const revenue = pendingOrders.reduce((sum, o) => sum + (o.totalAmount || 0), 0);
      
      setStats({ total, pending, processing, delivered, revenue });
      
      // Calculate vendor stats
      const vendorStatsData = calculateVendorStats(pendingOrders);
      setVendorStats(vendorStatsData);
      
      console.log('✅ Pending orders fetched successfully:', pendingOrders.length, 'orders');
  ...
  const handleStoreSelect = async (store) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setVendorStoreOrders(pendingOrders);
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto p-6">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage pending orders and vendor statistics</p>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <ExportButton data={filteredOrders} filename="pending-orders-export" />
            </div>
          </div>
            ...
            <div>
              ...
              <div className="bg-white rounded-lg shadow overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200">
                  <h3 className="text-lg font-medium text-gray-900">Vendor Pending Order Statistics</h3>
                  <p className="text-sm text-gray-600 mt-1">Performance metrics for each vendor and their stores (pending orders only)</p>
                </div>
                  ...
                  {vendorStats.length === 0 && (
                    <div className="text-center py-12">
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        No pending orders found to calculate vendor statistics.
                      </p>
                    </div>
                  )}
                </div>
          ...
          {activeTab === 'vendor-orders' && selectedVendor && selectedStore && (
            <div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-medium text-gray-900">
                      Pending Orders for {selectedVendor.vendorName} - {selectedStore.name}
                    </h3>
                    <p className="text-sm text-gray-600 mt-1">
                      {vendorStoreOrders.length} pending orders found
                    </p>
                  </div>
                  <button
              ...
              <DataTable
                ...
                searchable={true}
                pagination={true}
                emptyMessage="No pending orders found for this vendor store"
              />
            </div>
          )}
