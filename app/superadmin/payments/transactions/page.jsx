    ...
    try {
      ...
      if (ordersResponse.success) {
        ...
        const transactionsData = ordersData.map(order => ({
          ...
          customerName: order.deliveryAddress?.fullName || order.shippingAddress?.fullName || 'Unknown Customer',
          customerEmail: order.deliveryAddress?.email || order.shippingAddress?.email || 'No Email',
          amount: order.totalAmount || 0,
          paymentMethod: order.paymentMethod || order.paymentDetails?.method || 'Unknown',
          status: order.paymentStatus || order.paymentDetails?.status || order.status,
          gateway: order.paymentDetails?.gateway || 'Stripe',
