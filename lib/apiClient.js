...
const getAuthBaseUrl = () => {
  const url = process.env.NEXT_PUBLIC_AUTH_API_URL || 'http://localhost:8888/api/auth';
...
class ApiClient {
  ...
  getAuthToken() {
    if (typeof window !== 'undefined') {
      ...
      console.log('🔍 [ApiClient] getAuthToken - tokensData:', tokensData ? 'exists' : 'null');
      if (tokensData) {
        try {
          ...
          console.error('❌ [ApiClient] Error parsing tokens from localStorage:', error);
          return null;
        }
      }
    }
